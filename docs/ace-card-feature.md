...
6. **Starting Card Exception**: When an Ace IS the starting card, no suit selection is triggered - any card can be played on it (the Ace acts as a wild starting card)
...
5. **Ace Starting Card**: When an Ace is the starting card, `action_type` and `action_suit` remain nil - no suit selection is triggered, any card can be played on it (the Ace acts as a wild starting card)
...
9. ✅ **Ace as starting card has no suit restriction** - no `action_type` or `action_suit` set, any card can be played
