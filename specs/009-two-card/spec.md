...
- Q: What happens when a player plays the '2' as their last card and becomes cardless? → A: The penalty still applies to the next player. The player who played the last '2' becomes cardless, but the next player must draw 2 cards (or block) as normal.
...
### User Story 5 - Player has no blocking cards and must draw 2 cards (Priority: P2)

As a player facing a '2' card penalty, if I have no Ace or '2' card to block it, I must draw 2 cards from the deck and end my turn.

**Why this priority**: This enforces the penalty mechanism and ensures the rule is applied consistently.

**Independent Test**: Create a scenario where a player faces a '2' penalty with no blocking cards and verify they draw 2 cards automatically.

**Acceptance Scenarios**:

1. **Given** the previous player played a '2' and the current player has no Ace or '2' cards, **When** it becomes the current player's turn, **Then** they automatically draw 2 cards from the deck, their turn ends, and the draw penalty is cleared.
2. **Given** a player must draw 2 cards but the deck has only 1 card, **When** the penalty is applied, **Then** the player draws the 1 available card, the played cards are recycled to form a new deck, and the player draws the second card from the new deck.
3. **Given** a player must draw 2 cards but the deck is empty and there are cards to recycle, **When** the penalty is applied, **Then** the played cards are recycled, and the player draws 2 cards from the new deck.

---

### Edge Cases

- What happens if a '2' is played as the very last card? The '2' card is NOT allowed as a valid finishing card. When a player plays the '2' as their last card, they become cardless, but the penalty still applies to the next player, who must draw 2 cards (or block) as normal.
...
- **FR-004**: If the next player has no Ace or '2' card to block the penalty, the system MUST automatically make them draw 2 cards from the deck when their turn begins, end their turn, and clear the draw penalty.
- **FR-005**: When a player plays multiple '2' cards in a single turn (combo), the system MUST apply only a single 2-card draw penalty to the next player (not additive).
- **FR-006**: If a requested suit is active (from an Ace) and a player plays a '2', the '2' MUST match the requested suit to be valid. Playing a valid '2' MUST clear the requested suit requirement and apply the draw penalty.
- **FR-007**: If both a draw penalty and a requested suit are active, the blocking card (Ace or '2') MUST match the requested suit. If no valid card can be played, the player MUST draw 2 cards and the requested suit MUST persist.
- **FR-008**: When the draw deck has fewer than 2 cards and a player must draw 2, the system MUST: (a) draw available cards, (b) recycle the play pile (keeping the top card), (c) draw remaining cards from the new deck.
- **FR-009**: If the draw deck is empty and there are no cards to recycle (only the top card remains on the play pile), the system MUST log this anomaly, the player draws 0 cards, and their turn is skipped.
- **FR-010**: The system MUST exclude rank '2' cards when selecting the starting card during game initialization.
- **FR-011**: When a player is forced to draw 2 cards due to a '2' penalty, the system MUST display a notification/message explaining the reason (e.g., "You must draw 2 cards due to [Player]'s '2' card").
- **FR-012**: When a '2' penalty is active, the system MUST display a persistent visual indicator on the game board visible to all players (e.g., badge/icon showing "Draw 2 penalty active").
...
- **SC-005**: User session recordings show that players successfully use the '2' card to penalize opponents and use blocking cards (Ace or '2') without errors.
...
- After a player draws 2 cards due to the penalty, their turn ends. They cannot play any of the drawn cards in the same turn.
