...
%{
  active: boolean,
  penalty_type: string,    # "two" or "three" - determines count
...
if game_session.draw_penalty["active"] do
  # Clear penalty regardless of type
  draw_penalty = %{active: false, penalty_type: nil}
...
IO.puts("Found #{length(game_sessions)} game sessions to migrate")

Enum.each(game_sessions, fn gs ->
  updated_penalty = 
    cond do
  ...
  IO.puts("Migrated game session #{gs.id}")
end)
